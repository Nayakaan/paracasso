...
<?php
    ...
    query_posts($fargs);
    if(have_posts()): ?>
        <div class="row <?php echo ($is_filter == 'yes' ? 'shaff_grid' : 'grid'); ?>">
            <?php
            $sm = array(3, 4, 6, 7, 13, 14);
            $lg = array(5, 10, 15, 20, 25);
            $i = 1;
            while(have_posts()):
                the_post();
                ...
                }elseif(in_array($i, $lg)){
                   $c = 'col-lg-6 col-md-12';
                   $w = 895;
                   $h = 301; 
                ...
                ?>
                <div class="<?php echo esc_attr($c); ?> <?php if($is_filter == 'yes'): ?>shaff_item<?php else: ?>grid-item<?php endif; ?>" <?php if($is_filter == 'yes'): ?>data-groups='<?php echo esc_attr($groups); ?>'<?php endif; ?>>
                    <div class="gallerItem_2">
                        <img src="<?php echo makeover_post_thumbnail(get_the_ID(), $w, $h); ?>" alt="<?php echo get_the_title(); ?>">
            ...
            endwhile;
            ?>
            <?php if($is_filter == 'yes'): ?><div class="col-md-1 shaf_sizer"></div><?php else: ?><div class="col-md-1 grid-sizer"></div><?php endif; ?>
        </div>
